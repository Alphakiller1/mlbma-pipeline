...
| D-05 | Real 404 page still missing | mlbma | **not started** | WP5-1 | Unknown URLs currently 200-catch-all or blank |
...
| D-10 | `Last_Updated` triple fetch + `chase_nav` clock fallback | mlbma | **not started** | `dashboard/index.html` tab fetch; `team_profile.html` gviz probe; `matchup_shared.js` / `mlbma_ui.js`; `chase_nav.js` fallback | WP3 replaces three fetchers with DataStatus |
...
| WP0-R4 | Nav still points at OEM filename | mlbma | **not started** | `dashboard/index.html` chase-nav links `chase_analytics_mlb_oem_v7.html` | Extra stub redirect; WP5/nav carve-out |
