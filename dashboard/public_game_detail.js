...
(function (global) {
  ...
  var NFL_MATCHUP_ROWS = [
    ...
    ['off_first_down', 'def_first_down', 'First-Down Rate',
      'Share of plays that became a first down', 'Drive length and play volume'],
    ['off_explosive', 'def_explosive', 'Explosive-Play Rate',
      'Share of plays that cleared the source explosive-play threshold',
      'Field position and scoring volatility'],
    ['off_sack', 'def_sack', 'Sack Rate',
      'Sacks taken by the offense versus sacks generated by the defense',
      'Passing rhythm and route volume'],
    ['off_turnover', 'def_turnover', 'Turnover Rate',
      'Giveaways by the offense versus takeaways generated by the defense',
      'Possession count and short fields']
  ];

  var NFL_SCRIPT_LENSES = [
    {
      title: 'Scoring Efficiency', metric: 'EPA / Play', off: 'off_epa', def: 'def_epa',
      impact: 'Shows whether ordinary snaps have created or prevented expected points. Sustained efficiency changes how aggressively the trailing team may need to play.'
    },
    {
      title: 'Drive Control', metric: 'First-Down Rate', off: 'off_first_down', def: 'def_first_down',
      impact: 'First downs extend possessions and create more carries, routes and red-zone opportunities. Resistance can force longer third downs and shorter drives.'
    },
    {
      title: 'Explosive Volatility', metric: 'Explosive-Play Rate', off: 'off_explosive', def: 'def_explosive',
      impact: 'Chunk plays flip field position and create scoring without long drives, increasing the range of plausible game states.'
    },
    {
      title: 'Passing Friction', metric: 'Sack Rate', off: 'off_sack', def: 'def_sack',
      impact: 'Sacks end possessions and disrupt longer-developing routes. Clean pockets preserve dropback volume; pressure can compress it.'
    },
    {
      title: 'Possession Swings', metric: 'Turnover Rate', off: 'off_turnover', def: 'def_turnover',
      impact: 'Turnovers remove offensive opportunities and can hand the opponent a short field. This is a volatility signal, not a turnover prediction.'
    }
  ];
  ...
  function nflMatchupDirection(sport, game, offSide, defSide) {
    var rows = NFL_MATCHUP_ROWS.map(function (spec) {
      ...
      var leadLabel = lead === ' is-off' ? 'Offense rates stronger' :
        (lead === ' is-def' ? 'Defense rates stronger' : 'Indicators level');
      return '<div class="ca-nfl-matchup-row' + lead + '">' +
        nflFormCell(offense, 'Offense') +
        '<div class="ca-nfl-matchup-axis"><strong>' + esc(spec[2]) + '</strong>' +
        '<span>' + esc(spec[3]) + '</span><small>Game script · ' + esc(spec[4]) + '</small>' +
  ...
  function nflReadingKey() {
    return '<aside class="ca-nfl-reading-key" aria-label="How to read the matchup board">' +
      ...
      '<p>Every rank uses the same 32-team pool and is normalized so 1st is strongest. ' +
      'The highlighted side has the better season-to-date indicator; it is evidence about the matchup, not a forecast or recommendation.</p></aside>';
  }

  function nflScriptReading(sport, game, offSide, defSide, lens) {
    var offense = nflFormEntry(game, offSide, lens.off);
    var defense = nflFormEntry(game, defSide, lens.def);
    if (!offense && !defense) return '';
    var offPct = percentOf(offense);
    var defPct = percentOf(defense);
    ...
    if (offPct != null && defPct != null) {
      ...
      verdict = Math.abs(gap) < 8 ? 'The league-relative indicators are closely matched.' :
  ...
  function nflScriptLens(sport, game) {
    var cards = NFL_SCRIPT_LENSES.map(function (lens) {
      var directions = nflScriptReading(sport, game, 'away', 'home', lens) +
        nflScriptReading(sport, game, 'home', 'away', lens);
    ...
    if (!cards) return '';
    return '<section class="ca-script-dynamics" aria-labelledby="caScriptDynamicsTitle">' +
      '<header><div><span>Competitive Dynamics</span><h3 id="caScriptDynamicsTitle">How The Evidence Can Shape Game Script</h3></div>' +
      '<p>Translate efficiency into possessions, pressure and opportunity—without turning descriptive evidence into a prediction.</p></header>' +
  ...
  function nflDecisionPaths() {
    return '<nav class="ca-research-paths" aria-label="Choose a research path">' +
      '<a href="#form"><strong>Game Bettors</strong><span>Unit-vs-unit efficiency, volume and league rank</span></a>' +
      '<a href="#players"><strong>Props &amp; Fantasy</strong><span>Workload, production and coverage response</span></a>' +
      '<a href="#availability"><strong>Availability</strong><span>Starting units and official injury designations</span></a>' +
      '</nav>';
  }
  ...
  function nflSections(sport, game) {
    ...
    return [
      section('form', 'Matchup Breakdown',
        'Efficiency, Standard Production And Opponent Context',
        nflDecisionPaths() +
        nflReadingKey() +
        ...
        '</div>' + nflScriptLens(sport, game) +
        '<div class="ca-production-head"><div><span>Standard Production</span>' +
        '<h3>Yards, Touchdowns And Drive Volume</h3></div>' +
        '<p>Raw results beside rate-based efficiency</p></div>' +
        ...
        nflMetricGuide() +
        '<p class="ca-detail-source-note">The primary boards pair each offense with the defense it actually faces. ' +
        'Ranks are recomputed from the displayed observed rates; the brighter side is the stronger league-relative ' +
        'indicator on that row, not a pick, projection or recommendation. ' +
        (source.season ? 'Team form: ' + esc(source.season) +
          (source.week ? ', through week ' + esc(source.week) : '') + '. ' : '') +
        'Standard production is completed-game regular-season output from nflverse/nflfastR.</p>'),
